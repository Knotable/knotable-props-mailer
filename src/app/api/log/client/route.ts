...
const MAX_BODY_BYTES = 32_768; // 32 KB — prevent log flooding / payload stuffing
...
export async function POST(request: Request) {
  // ── Rate limit: 10 log events per IP per minute ──────────────────────────
  const ip =
  ...
  // ── Resolve optional user from session ───────────────────────────────────
  ...
  const correlationId = await logError({
    ...
    payload: parsed.data.context as Record<string, unknown> | undefined,
