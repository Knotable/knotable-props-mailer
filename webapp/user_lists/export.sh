...
-p ${password} --eval "rs.slaveOk();db.users.count()")

#Users who's last seen was in the past 30 days
activeusers=$(mongo ${extraArgs} --quiet ${ip}/${dbname} -u ${username} \
-p ${password} --eval 'rs.slaveOk();db.users.count({ "last_seen": { "$gt": ISODate("'${lastmonth}'")}})')

userstoday=$(mongo ${extraArgs} --quiet ${ip}/${dbname} -u ${username} \
-p ${password} --eval 'rs.slaveOk();db.users.count({ "last_seen": { "$gt": ISODate("'${yesterday}'"), "$lt": ISODate("'${today}'")}})')

signupstoday=$(mongo ${extraArgs} --quiet ${ip}/${dbname} -u ${username} \
-p ${password} --eval 'rs.slaveOk();db.users.count({ "createdAt": { "$gt": ISODate("'${yesterday}'"), "$lt": ISODate("'${today}'")}})')

userswithpass=$(mongo ${extraArgs} --quiet ${ip}/${dbname} -u ${username} \
-p ${password} --eval 'rs.slaveOk();db.users.count({ "services.password": { "$exists": true } })')

#total users with at least 3 pads
userswithpads=$(mongo ${extraArgs} --quiet ${ip}/${dbname} -u ${username} -p \
${password} --eval 'rs.slaveOk();db.users.count({ "statistics.numberOfActivePads": { $gt: 2 } })')

totalpads=$(mongo ${extraArgs} --quiet ${ip}/${dbname} -u ${username} \
-p ${password} --eval 'rs.slaveOk();db.topics.count()')

totalknotes=$(mongo ${extraArgs} --quiet ${ip}/${dbname} -u ${username} \
-p ${password} --eval 'rs.slaveOk();db.knotes.count()')

#total replyByEmail events grouped by date_created (to second)
emailreplies=$(mongo ${extraArgs} --quiet ${ip}/${dbname} -u ${username} -p \
${password} --eval 'rs.slaveOk();db.knotable_events.aggregate([ { $match: { "event_type": "replyByEmail" } }, { $group: { "_id": { month: { $month: "$date_created"}, day: { $dayOfMonth: "$date_created"}, year: { $year: "$date_created"}, hour: { $hour: "$date_created"}, minute: { $minute: "$date_created"}, second: { $second: "$date_created"}} } } ])')

#contact usernames in contactAddedToPad event
contactadded=$(mongo ${extraArgs} --quiet ${ip}/${dbname} -u ${username} -p \
${password} --eval 'rs.slaveOk();db.contacts.count()')
