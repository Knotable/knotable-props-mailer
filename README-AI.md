...
For Next.js behavior, read the version-matched bundled documentation under `node_modules/next/dist/docs/`; this checkout targets Next.js `16.3.4`. Do not rely on older framework conventions from memory.
...
The reviewed canonical content for these reminders removed the street address `2 Desbrosses Street, New York`, replaced the venue detail with `Location — Shared with registered guests`, changed `Program — Begins at 10:00am`, and changed the footer strapline to `AI x Health`. Draft variants cloned before these corrections were stale and must not be used. During this incident, stale/autosaved variants also appeared with `error ` prefixed subjects and inconsistent or absent `email_recipients` bindings. Amol deleted the obsolete drafts; future operators must identify a draft by exact UUID and audience binding, not merely by subject. Before queueing any derived campaign, compare its HTML and text with the approved canonical source and verify all four invariants: no street address, 10:00am program start, `AI x Health` footer, and the intended list address.

**What the production UI actually does:** Composer **Queue** saves the draft and materializes one `mail_queue` row per deduplicated active member, held at year 2999. It may display a legitimate recent/duplicate-send warning; for a deliberate reminder, review the overlap and use **Queue anyway** only when the operator has explicitly approved re-contact. The queue total may be one greater than the active list count because Props Mailer automatically adds `a@sarva.co` as a direct owner copy. Treat that as a real recipient and include it in the approval/count. On the Queue screen, **Resume / Confirm resume** changes the parent email to `sending` and opens the scoped monitor; it does **not** launch the durable SES worker. The Monitor page is status-only. The global `/email/monitor` explicitly never drains rows, and opening, closing, or refreshing either monitor does not start, stop, or sustain delivery. A campaign can therefore display `Sending` while every pending row is still held and SES accepted remains zero.

**What actually sends:** after the exact queue count, content, sender, list, and owner copy are approved, dispatch `.github/workflows/ses-bulk-worker.yml` for that single campaign UUID with `mode=send`, confirmation `send:<emailId>`, and a reviewed `max_recipients_per_second`. The September newsletter used `13/second`, below the verified SES maximum of `15/second`; the worker also performed configured recovery pauses. Verify the GitHub run reaches `in_progress` and then confirm the first `mail_queue.status = succeeded` rows before saying that delivery has started. Do not infer progress from the global monitor's `Sent last 7 days`, which is a historical aggregate. At terminal completion require `pending=0`, `processing=0`, a successful worker conclusion, and separately report failed/dead/canceled. Never blindly rerun a campaign with `processing > 0`; reconcile ambiguous claims first.

**Concurrency and quota rule:** never launch two campaign workers concurrently. SES quota is shared and rolling (`65,400` recipients per 24 hours at the last verified limit), while separate campaign workers can each observe the same apparent headroom. Serialize campaigns, wait for the first worker to become terminal, then re-read live SES headroom before dispatching the next. A `184,983`-recipient audience cannot fit in one quota window or the legacy worker's six-hour ceiling. Three equal batches would be about `61,661` recipients and would not preserve the 20% policy buffer under the current quota; use at least four immutable batches of roughly `46,246` recipients, with exact counts derived from the fresh deduplicated snapshot. Do not materialize or send that Amolworld campaign through the legacy browser/Vercel path. Complete the AWS-native deployment and canaries described below before its production send.

**Product gaps exposed by this incident:** add a canonical **Clone campaign** action; show a source-to-draft HTML/text diff; store event date, program start, venue visibility, and footer as structured fields; flag street addresses in broad campaigns; expose an explicit **Send owner copy** toggle; show the final deduplicated queue count before confirmation; distinguish `Held`, `Worker queued`, `Sending`, `SES accepted`, and `Delivered`; dispatch the durable worker from the authenticated app; default Monitor to one campaign and segregate historical aggregates; implement a global SES quota lease so workers cannot oversubscribe shared headroom; provide adaptive pacing/ETA and terminal notifications; and bind approval to an immutable content-and-audience digest. The Sends page also stalled on Supabase during lookup, so campaign identity/history queries need the same bounded, failure-explicit treatment as analytics.

**2026-09-10 postmortem guardrails implemented locally (deployment pending):** Queue creation now writes an immutable content snapshot and puts only its identifier plus a SHA-256 digest on each legacy `mail_queue` payload. The worker verifies the digest and sends from that frozen snapshot rather than a subsequently edited mutable draft. The previous implicit owner copy is replaced by an explicit, opt-in **Send owner copy** control; its message makes the extra recipient visible during preparation. Queue/readiness blocks likely precise street addresses with guidance to use “Location shared with registered guests.” Campaign-scoped Monitor labels now distinguish **Held**, **Preparing audience**, **Worker queued**, **Sending**, and terminal **SES accepted** states; account-wide historical figures are explicitly labeled and segregated. These are source changes only: do not claim an immutable audience digest, app-triggered legacy-worker dispatch, visual derived-draft diff, structured event fields, quota lease, or AWS production authorization until their distinct release gates are completed.

**Concise next-time sequence:** (1) identify the exact canonical source UUID; (2) clone one new draft per audience; (3) verify sender, reply-to, subject, HTML, text, list binding, privacy-sensitive location details, schedule, footer, and owner-copy behavior; (4) recount active and final deduplicated recipients; (5) review duplicate/recent-send evidence; (6) queue and verify all rows are held with `processing=0`; (7) obtain exact approval for the final queue count and immutable content; (8) set the parent to `sending`; (9) dispatch exactly one scoped durable worker; (10) verify first acceptances, monitor GitHub plus campaign-scoped queue counts, and do not launch another campaign until terminal; (11) report SES acceptance separately from provider delivery, bounce, complaint, open, and click events.

**2026-09-04 LAZY ANALYTICS LIVE:** The production 50-campaign aggregate returned PostgreSQL `57014`; the former banner incorrectly called that a missing migration and substituted zeros. The page now fetches only lightweight campaign identities, refreshes one campaign and one metric at a time, stores complete 15-minute browser snapshots, and displays explicit waiting/loading/updated/error states. The service-role-only `20260904_lazy_campaign_analytics.sql` migration was applied transactionally through the Supabase session pooler. `npm run check:analytics -- --email-id=4ea3511e-64e7-40a0-94af-08d5381bd110` then passed all six calls: accepted `12,399`, pending `0`, failed `0`, delivered `12,196`, unique opened `6,652`, unique clicked `3,100`, bounced `410`, complained `0`; provider events were fresh. `20260904_set_based_recent_analytics.sql` is optional. Future AWS-native status reads DynamoDB via `/email/aws-native`.

**2026-09-04 THREE-BATCH RELEASE IMPLEMENTATION:** Manifest schema v2 canonically partitions recipient ordinals into three non-overlapping batches. The snapshot builder refuses an email with any legacy `mail_queue` rows and initializes only four AWS control records, not one row per recipient. The native worker requires `--batch-number` plus `send:<campaignId>:<digest-prefix>:batch:<number>`, accepts only `READY_FOR_APPROVAL`/same-token `APPROVED`, checks rolling quota for the complete remaining batch plus one notice, maintains atomic per-batch counters with recipient checkpoints, and cannot cross ordinal boundaries. Completion atomically changes the next batch from `QUEUED_FOR_APPROVAL` to `READY_FOR_APPROVAL`; it does not send it. The worker conditionally claims the operator notice once, records an ambiguous outcome rather than retrying automatically, and includes a return-to-app URL. Each recipient gets signed HTTPS `List-Unsubscribe` and `List-Unsubscribe-Post` headers. `/email/aws-native` reads DynamoDB/SES status and dispatches a separately confirmed GitHub worker; Vercel stores no execution state. The workflow uses OIDC syntax and no static AWS keys. **Still blocked:** the CloudFormation OIDC role and public HMAC-validated unsubscribe Lambda URL are persistent security boundaries requiring Amol's explicit approval; they are not added or deployed. The existing audience builder can upload subscriber PII to private S3 only as a deliberate operator command; automating that export also requires explicit approval.

**2026-09-04 AUTH BYPASS HARDENING:** The repository no longer contains live bypass-password or cookie-HMAC constants. `BYPASS_PASSWORD_SHA256` and `BYPASS_COOKIE_HMAC_KEY` must both be valid 64-character hex values or bypass authentication is disabled in both Node and Edge paths. The bypass action now allows at most five attempts per 15 minutes per app instance. Before production use, create a new long password and independent random HMAC key in Vercel; never reuse the retired values in git history. Keep `AWS_NATIVE_CONTROL_ENABLED=false` until this and the AWS cutover gates are complete.

**2026-09-04 RELEASE DEPENDENCY HARDENING:** Next.js was upgraded from `16.2.2` to patched `16.3.4`, Nodemailer from `6.9.13` to `10.0.0`, Vitest from `2.1.8` to `5.0.0`, and Vite is pinned at `8.2.2`; Node types now target 22. The production build and 49 tests pass, lint has zero errors (three pre-existing warnings), and `npm audit` reports zero vulnerabilities. `next-env.d.ts` contains the Next 16.3 generated route/root-parameter references; do not revert them while this version is installed.

**2026-09-04 PRIOR CAMPAIGN TERMINAL:** Amol explicitly confirms campaign `4ea3511e-64e7-40a0-94af-08d5381bd110` completed and should be treated as terminal. The live analytics verification above independently found `12,399` accepted with `0` pending and `0` failed; provider events were fresh. The next intended send is approximately `150,000` recipients, with no browser tab and no Vercel or Supabase persistence in the delivery hot path.

**2026-09-03 preceding incident—closed historical context:** campaign `4ea3511e-64e7-40a0-94af-08d5381bd110` ("LifeX AGM: Health AI’s leading CEOs + Dr. Nir Barzilai", LifeX newsletter, `12,399` recipients) was released on 2026-09-02. A browser/Vercel worker initially stopped after claiming `35` recipients without recording outcomes, leaving `12,364` pending at that snapshot. The later durable worker completed the send; the live 2026-09-04 analytics verification and Amol's terminal confirmation supersede the incomplete incident snapshot.
...
| Commit/persist this operator status | Done 2026-09-10 | The final-reminder postmortem, exact campaign outcomes, UI/worker semantics, concurrency rule, and next-time runbook are committed in the canonical guide. |
...
| Analytics reliability + iPhone mode | Lazy implementation ready 2026-09-04 | The bulk RPC is installed but its 50-row aggregate exceeds statement timeout (`57014`). The page now renders campaign identities immediately, refreshes one campaign/metric at a time, caches complete rows for 15 minutes, labels work/failures, and never substitutes zero for unavailable values. Apply `20260904_lazy_campaign_analytics.sql` and run the revised sequential `npm run check:analytics`; AWS-native status must still move to DynamoDB/S3. |
...
private versioned S3 campaign.json + recipients.ndjson.gz
        |
        | three fixed batch records; exact digest + batch approval token
        v
...
| SES daily quota cuts a campaign in half | One manifest is pre-partitioned into three fixed release batches. Send mode fails closed unless live rolling quota headroom covers every recipient in the explicitly approved batch; it never spills into the next one. |

### Implemented locally through 2026-09-04 (AWS plane not deployed)
...
- `scripts/ses-native-worker.mjs` downloads and verifies that package before any send, reads live SES quota/rate, requires a digest-and-batch-bound token, refuses partial-batch quota, acquires a DynamoDB lease, conditionally claims at most 25 recipients inside the selected ordinal range, sends with SES v2, and atomically checkpoints recipient plus batch counters. It never crosses into another release batch.
- `.github/workflows/ses-native-worker.yml` runs the native worker for up to six hours with a per-campaign concurrency lock; dry-run is the default, an exact batch number is mandatory, GitHub OIDC is required, and static AWS keys are not accepted by the workflow.
- `infra/ses-native-worker/cloudformation.yml` currently defines a private versioned S3 bucket, encrypted/PITR DynamoDB table, SES configuration set, SNS event destination, and Lambda event consumer. The consumer updates latest provider state, creates global bounce/complaint suppressions, and writes each raw event as immutable S3 JSON. The least-privilege GitHub OIDC role, Secrets Manager key, and HMAC-validating unsubscribe Lambda URL are deliberately not yet in this file because their persistent security boundary awaits explicit approval.
...
- `/email/aws-native` is the authenticated status/control page. It reads campaign/batch state from DynamoDB, reads current SES quota, re-verifies the manifest key before dispatch, and submits the existing GitHub worker. Vercel is a transient initiator, never the execution checkpoint.
- The AWS control page and both server actions are additionally fail-closed behind `AWS_NATIVE_CONTROL_ENABLED=true`; leave it false until the cloud stack, narrowly scoped credentials, live readiness check, and canaries are complete.
- Unit/static safety coverage lives in `scripts/lib/ses-native-core.test.mjs`, `scripts/lib/ses-bulk-worker-core.test.mjs`, and `src/lib/awsNativeCampaign.test.ts`.
- `npm run check:150k` is a read-only acceptance test for repository gates and prints the deterministic three-batch plan. `npm run check:150k -- --live` additionally checks SES production access, per-batch quota/headroom, send rate, S3 versioning, DynamoDB/PITR, configuration-set events, unsubscribe secret, and the fail-closed unsubscribe endpoint. It never calls an SES send API. Static checks now pass for batching, notification, and app control; they remain red for the undeployed OIDC role and unsubscribe endpoint.

### Not yet done—the actual production cutover

1. **Prior campaign terminal—done.** Amol confirms `4ea3511e-64e7-40a0-94af-08d5381bd110` sent. Live bounded analytics found accepted `12,399`, pending `0`, failed `0`, with fresh provider events.
2. **Operate within the existing SES capacity.** Do not pursue a quota increase. Require at least `50,001` unused rolling headroom before each 50k release (batch plus its operator notice) and keep the worker at no more than `13/second`, below 90% of the last verified `15/second` grant. Expect roughly 24 hours between batches at the fixed `65,400/24h` rolling quota.
...
4. **Configure GitHub and the app.** Read-only inspection on 2026-09-04 found only `AWS_REGION=us-east-1`; the native bucket/table/configuration-set/role/unsubscribe/operator/app variables are absent. Static `AWS_ACCESS_KEY_ID` and `AWS_SECRET_ACCESS_KEY` secrets still exist, but the revised native workflow deliberately ignores them. After explicit security-boundary approval, deploy the narrow OIDC role and set `AWS_SES_NATIVE_ROLE_ARN`, `AWS_SES_NATIVE_CONFIGURATION_SET`, `SES_CAMPAIGN_BUCKET`, `SES_CAMPAIGN_STATE_TABLE`, `SES_UNSUBSCRIBE_BASE_URL`, `SES_UNSUBSCRIBE_SECRET_ARN`, `SES_OPERATOR_EMAIL`, and `SES_APP_URL`. Give Vercel only read access to the campaign table and SES quota plus a fine-grained Actions-dispatch token; set `AWS_NATIVE_CONTROL_ENABLED=true` only after live readiness and canaries pass.
...
6. **App control and batch state machine—implemented, deployment pending.** `/email/aws-native` shows exact digest/audience/batches/quota and requires a batch-specific typed token. The worker persists batch 1 as `READY_FOR_APPROVAL`, later batches as `QUEUED_FOR_APPROVAL`, and promotes only the next batch to ready after terminal reconciliation. No state transition sends the next batch without another app approval.
7. **One-click unsubscribe—worker side implemented; endpoint approval pending.** Recipient-specific HMAC URLs are emitted in both `List-Unsubscribe` and `List-Unsubscribe-Post`. Explicitly approve, add, and deploy the proposed public AWS Lambda URL: GET only shows confirmation; POST requires the signed recipient/campaign query and exact RFC 8058 body, then writes only the hashed/email `!SUPPRESSION` item in the campaign DynamoDB table.
8. **Terminal completion notice—implemented, deployment pending.** After every recipient in the batch is terminal and `CLAIMED=0`, `RETRY=0`, the worker atomically promotes the next batch to ready, conditionally claims the notice once, and emails campaign/digest/batch/outcome plus the app link. An ambiguous SES notice outcome is recorded and never retried automatically.
9. **Run graduated canaries.** Use the exact production HTML and configuration set for `100`, then `1,000`, then `10,000` opted-in recipients. Exercise the completion email and manual promotion during these canaries. Stop on abnormal bounce/complaint, rendering, event-loss, duplicate-notice, or batch-boundary signals. Only then freeze the 150k manifest.
10. **Retire the legacy hot path.** After two successful AWS-native campaigns, disable large-list creation of Supabase `mail_queue` rows and mark `.github/workflows/ses-bulk-worker.yml`, monitor auto-drain, and Vercel worker endpoints as legacy repair-only. Preserve read-only historical reporting.
11. **Move the runner into AWS.** GitHub Actions is an acceptable bridge, not the ultimate scheduler. Add Step Functions/ECS Fargate when the app launch path is ready, or sooner if sends may exceed the six-hour Actions ceiling.

### 150k capacity arithmetic and gates

- Deterministic partitioning by immutable recipient ordinal yields `50,000 + 50,000 + 50,000` for exactly 150k. For uneven totals, assign the remainder one recipient at a time starting with batch 1; the three batch counts must sum exactly to the manifest count and no recipient may appear twice.
- A 50k batch plus the 20% quota policy requires `60,000/24h` configured quota and at least `50,001` current rolling headroom for the recipients plus completion notice. Capacity is re-evaluated immediately before every batch, so unrelated sends may defer the next approval even though the account's maximum quota is adequate.
- At the last verified `15 recipients/second`, the worker's 90% safety ceiling is `13.5/second`; one 50k batch takes about `62 minutes`. The default readiness policy therefore uses a 75-minute ceiling and requires at least `13/second`.
- The 24-hour SES quota is rolling, not a midnight reset. At `65,400/24h`, successive 50k batches will ordinarily be approximately 24 hours apart unless prior sends age out sooner or AWS grants more capacity.
...
- Each release requires live quota headroom for the entire approved batch. The worker must never “send as much as possible,” cross a batch boundary, or infer authorization for the next batch.
...
The default 150k policy plans `3` approval-gated batches, requires at least `60,000/24h`, enough current rolling headroom for the largest whole batch (`50,000`), and at least `13/second` so the worker's 90% ceiling can finish that batch within 75 minutes. Override only for diagnosis with `--audience=...`, `--batches=...`, `--max-minutes=...`, or `--quota-buffer=...`; changing a test threshold does not change the real SES grant or authorize a send.
...
Run **SES Native Worker** in GitHub with `mode=dry-run`, the exact campaign UUID, emitted immutable `manifest_key`, and exact `batch_number`. The dry run verifies S3 SHA-256/count, merge rendering, DynamoDB batch state, unsubscribe signing secret, operator-notice destination, and live SES capacity; it prints `send:<campaignId>:<digest-prefix>:batch:<number>`. Only after a human rechecks the campaign/body/audience/count may the same workflow—or `/email/aws-native`—be run with `mode=send` and that exact token.
...
4. Deterministically partition the immutable recipient ordinals into three non-overlapping release records. Only batch 1 begins `READY_FOR_APPROVAL`; later batches remain `QUEUED_FOR_APPROVAL`.
5. Run the GitHub **SES Native Worker** in `dry-run` for the selected batch. It verifies the whole artifact and batch boundary, reads DynamoDB state, and checks live SES quota/rate for the complete batch.
6. With exact approval, run `send` using a token bound to campaign digest and batch number. The runner persists claims/checkpoints in DynamoDB and needs no open tab, Vercel function, or Supabase write. It cannot cross into the next batch.
...
The campaign table is intentionally lazy. Initial render fetches only campaign identities. The client then refreshes one row at a time and one metric per request in this order: queue, delivered, opened, clicked, bounced, complained. Each complete row is cached in the current browser for 15 minutes and displays its age; **Update now** moves a row to the front of the single refresh queue. The admin-only route is `/api/email/analytics/<emailId>?metric=<metric>`. Install `supabase/migrations/20260904_lazy_campaign_analytics.sql` before relying on it, and validate one real campaign without mutations:

```bash
npm run check:analytics
npm run check:analytics -- --email-id=EMAIL_ID_HERE
```
...
- The Analytics page uses bounded, campaign-scoped RPCs rather than an all-campaign aggregate. A failed metric remains unavailable instead of being rendered as zero. For AWS-native sends, DynamoDB/S3 will become the status authority; these Supabase metrics cover legacy history.
...
GITHUB_ACTIONS_DISPATCH_TOKEN # fine-grained, this repository Actions:write only
GITHUB_REPOSITORY          # defaults Knotable/knotable-props-mailer
GITHUB_ACTIONS_REF         # defaults master
SES_CAMPAIGN_BUCKET
...
- **Priority transition:** complete the production-cutover checklist in **AWS-Native 150k Send Architecture**. Three approval-gated batches, app dispatch/status, per-batch counters, per-recipient unsubscribe headers, and the at-most-once operator completion notice are implemented. Explicit approval/deployment of the narrow OIDC role and public signed unsubscribe endpoint, GitHub/Vercel variables, deliberate subscriber snapshot, and graduated canaries remain. Live SES quota/rate/headroom must be re-verified for each whole batch; do not request a quota increase.
...
- Analytics reliability + iPhone mode PRD lives at `docs/analytics-mobile-prd.md`. The production bulk summary RPC timed out on 2026-09-04. The lazy migration is now live and all six bounded metrics passed for the latest completed campaign. Keep unavailable UI totals blank on any future metric failure; `20260904_set_based_recent_analytics.sql` remains optional.
