...
**2026-08-10 Supabase overload, resolved; load-reduction migration applied 2026-08-11:** the Supabase project (region `us-east-2`) was unresponsive (HTTP 522 on Auth and PostgREST) during the afternoon UTC. Commit `1c52d82` deployed code-side load reductions: 60s-cached `/api/health` with a 5-minute banner recheck, no-op worker runs no longer insert `queue_metrics` rows, the SES webhook and client-log routes use the in-memory rate limiter, the webhook dedupe pre-select was replaced by reliance on a partial unique index, and `getServerAuthContext` is request-deduped. **`supabase/migrations/20260810_free_tier_load_reduction.sql` was applied by the operator on 2026-08-11 and reported successful**, so `provider_events_message_event_unique_idx` now exists live and the webhook's `23505` duplicate path is real again; the `error_logs` rate-limiter sentinels and no-op `queue_metrics` rows were purged, and the daily `props_mailer_cleanup` pg_cron job keeps them bounded (it also drops `source = 'auth'` logs older than 30 days — an intentional retention policy, not just cleanup). Recovery evidence for the overload itself: the SES bulk worker failed all morning UTC on `SES claim checkpoint was incomplete after retries: Failed to connect to database: {:error, :timeout}`, then drained a campaign clean at 16:05-18:16 UTC with `accepted=4089`, `failed=0`, `activeRemaining=0`. Direct `db.<ref>.supabase.co` is IPv6-only from this network; use the pooler at `aws-1-us-east-2.pooler.supabase.com` (user `postgres.<ref>`, port 5432 session / 6543 transaction). Session `SET statement_timeout` does not persist through the pooler — wrap long statements in a transaction with `SET LOCAL`.
...
| Commit/persist this operator status | Done 2026-08-10 | `README-AI.md` operator status is committed (`2f8881e`) and kept current in the repo, not local-only. Update it in the same commit as any structural or operational change. |
| Apply free-tier load reduction migration | Done 2026-08-11 (operator-applied) | Operator ran `20260810_free_tier_load_reduction.sql` in the Supabase SQL Editor and reported success. Verified in code beforehand: no live reader depends on the purged rows — every rate-limiter call site uses in-memory `checkRateLimitSync` (`src/lib/rateLimit.ts`), and `/api/health` never reads `queue_metrics`. Dedupe caveat: `provider_events.id` is a `uuid`, so the migration's `dup.id < pe.id` tiebreak keeps an arbitrary row per `(message_id, event_type)` pair, not the earliest despite the comment — counts are correct, but a surviving row's `received_at` may be an SNS retry timestamp rather than the original. Not independently re-verified against the live DB from the agent session (egress policy blocks `*.supabase.co`); confirm `indisvalid`/`indisready` on `provider_events_message_event_unique_idx` in the Dashboard if it matters. |
