...
> **AWS-native transition prompt (updated 2026-09-04):** Until the checklist in **AWS-Native 150k Send Architecture** is complete, proactively tell Amol at the start of the next substantive Props Mailer interaction: “The AWS-native 150k path is still not production-ready. We still need to deploy and verify the AWS stack/OIDC, implement three digest-bound 50k approval batches plus the idempotent completion email, add native app launch/status controls and one-click unsubscribe, prove live SES rolling headroom for the next whole batch, and pass 100/1,000/10,000 canaries.” Do not imply that the legacy Supabase-backed worker is ready for 150k. This prompt is deliberately durable so the remaining work is raised again on the next interaction.
...
**2026-09-04 LAZY ANALYTICS IMPLEMENTATION:** The production `get_recent_email_analytics_stats` RPC is installed but the 50-campaign aggregate returned PostgreSQL `57014 canceling statement due to statement timeout`; the former banner incorrectly called that a missing migration and substituted zeros. The page now fetches only the 50 lightweight campaign identities on initial render. In the browser it refreshes one campaign at a time, and within that campaign calls queue, delivered, opened, clicked, bounced, and complained metrics sequentially. Completed snapshots are timestamped in versioned `localStorage` for 15 minutes; rows display `Loading <metric>`, `Waiting to refresh`, `Updated <age>`, or an explicit failure plus **Update now/Retry**. The admin-only API uses service-role RPCs that are revoked from `public`, `anon`, and `authenticated`. Apply `20260904_lazy_campaign_analytics.sql`, then run `npm run check:analytics`; the checker exercises the same six bounded calls serially and never mutates or sends. `20260904_set_based_recent_analytics.sql` remains an optional bulk-query optimization, not a prerequisite for the lazy page. AWS-native campaign status must ultimately read DynamoDB/S3 rather than this legacy Supabase view.

**2026-09-04 THREE-BATCH 150K DECISION:** Amol accepts one approximately `150,000`-recipient campaign being deterministically partitioned into three releases, normally `50,000` each. The immutable campaign digest and recipient ordinals must define non-overlapping batches. Batch 1 may become `READY_FOR_APPROVAL`; batches 2 and 3 begin `QUEUED_FOR_APPROVAL`. After a batch has zero `CLAIMED`/`RETRY` recipients and all entries are terminal, one idempotent SES operator email should say the batch finished and link back to the app to approve the next batch. That email is notification, never authorization: no later batch auto-releases. Immediately before every approval, re-check suppression, configuration, rolling SES headroom for the whole batch, and the digest-bound confirmation. At the last verified `65,400/24h` and `15/second`, an otherwise unused account can theoretically accommodate one 50k batch in about 62 minutes at 90% utilization, but the rolling window ordinarily forces roughly 24 hours between releases. The current native worker still enforces all-or-nothing manifest quota and does not implement this batch state machine or notification; `npm run check:150k` must remain red for those gates until code and live evidence exist.

**2026-09-03 USER-REPORTED COMPLETION + AWS-NATIVE TRANSITION:** Amol reports that the last campaign finally sent. This has not been independently reconciled in this documentation update, so verify campaign `4ea3511e-64e7-40a0-94af-08d5381bd110` has terminal queue/provider state before treating it as confirmed history. The next intended send is approximately `150,000` recipients, with no browser tab and no Vercel or Supabase persistence in the delivery hot path. The repository contains the first implementation of that target: immutable S3 campaign packages, DynamoDB conditional recipient state, an autonomous GitHub Actions SES worker, and SES configuration-set events through SNS/Lambda into DynamoDB plus append-only S3 records. **Nothing in this new path is deployed or authorized to send yet.** The worker currently refuses a partial manifest; replace that behavior only with the explicit three-batch, approval-gated model above—not opportunistic “send as much as possible” behavior.
...
- `scripts/ses-native-worker.mjs` downloads and verifies that package before any send, reads live SES quota/rate, requires `send:<campaignId>:<digest-prefix>`, currently refuses partial-manifest quota, acquires a DynamoDB lease, conditionally claims at most 25 recipients, sends with SES v2, and atomically checkpoints results. The three-batch release state machine described below is the approved replacement for its current whole-manifest quota rule; it is not implemented yet.
...
- `npm run check:150k` is a read-only acceptance test for repository gates and prints the deterministic three-batch plan. `npm run check:150k -- --live` additionally checks SES production access, per-batch quota/headroom, send rate, S3 versioning, DynamoDB/PITR, and the live configuration-set events. It never calls an SES send API. The check is intentionally red until OIDC, approval-gated batches, the idempotent operator email, one-click unsubscribe, and the in-app AWS control/status surface exist.
...
2. **Verify per-batch SES capacity before 150k.** With the accepted three-batch plan, require at least `60,000/24h` configured quota (50k plus 20% policy buffer), at least `50,000` unused rolling headroom immediately before each release, and at least `13/second` for a 75-minute ceiling at 90% utilization. The last recorded `65,400/24h` and `15/second` can meet those numerical thresholds only when the rolling window is nearly unused; verify current grants and consumption with SES `GetAccount`. A request, old screenshot, or configured app limit is not live headroom.
...
6. **Add the app control surface and batch state machine.** Replace large-campaign **Queue/Send Now** with **Freeze AWS Campaign**; show the S3 digest, exact audience, three non-overlapping batch counts, current batch state, quota/headroom, and time estimate. Require a batch-number-and-digest-bound confirmation for each release through API Gateway/Lambda. Persist batch 1 as `READY_FOR_APPROVAL` and later batches as `QUEUED_FOR_APPROVAL`; only a human app action can promote and release the next batch. Status must read AWS state and must not revive the browser worker.
...
8. **Add the terminal completion notice.** Only after every recipient in the current batch is terminal and `CLAIMED=0`, `RETRY=0`, conditionally set a durable `operator_notice_sent_at` value and send one email to the configured operator address. The subject/body should identify campaign, digest, batch number, accepted/dead/canceled totals, and link to the app's next-batch approval screen. Retries must not duplicate the notice, and the notice must never release a batch.
...
- A 50k batch plus the 20% quota policy requires `60,000/24h` configured quota and at least `50,000` current rolling headroom. Capacity is re-evaluated immediately before every batch, so unrelated sends may defer the next approval even though the account's maximum quota is adequate.
...
- A higher quota can shorten the overall calendar schedule, but a higher rate is not necessary merely to make a 50k batch fit the current six-hour runner. Raise throughput only after event, suppression, and complaint monitoring pass canaries.
...
- **Priority transition:** complete the production-cutover checklist in **AWS-Native 150k Send Architecture**. The code is local; AWS infrastructure, GitHub variables, OIDC, three approval-gated release batches, the idempotent operator completion email, canaries, one-click unsubscribe, and the app control/status surface remain incomplete. Live SES quota/rate/headroom must be re-verified for each whole batch.
...
- Analytics reliability + iPhone mode PRD lives at `docs/analytics-mobile-prd.md`. The production bulk summary RPC timed out on 2026-09-04. The UI now refreshes sequential per-campaign metrics; apply `supabase/migrations/20260904_lazy_campaign_analytics.sql`, rerun `npm run check:analytics`, and keep unavailable UI totals blank until each bounded metric passes. `20260904_set_based_recent_analytics.sql` is an optional bulk-query optimization.
